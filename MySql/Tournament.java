...
public abstract class Tournament {
	...
	protected int[] prizes = new int[3];
	private boolean endOfTournament = false;

	
	private IMatchweekCreator mwc;
	
	public Tournament(String name,List<Team> teams , IMatchweekCreator mwc){
		this.name = name;
		this.teams = teams;
		this.mwc = mwc;
	...
	public void showRanking()  {
		...
		if(endOfTournament) {
			...
			for(int i=0; i < winners.length;i++) {
			 ...
			 System.out.println((i + 1) + "\t" + winners[i] + "\t" + this.getPrizes()[i]);
			
			mysql.insertQuery("INSERT INTO tourwinners (TourName,TeamName,Prize) VALUES('" + this.name + "'  ,'" + winners[i] + "'  ,'" + this.getPrizes()[i] +"')");
	...
	public abstract int[] getPrizes();

	public abstract void setPrizes(int[] prizes);
	
	public abstract void accept(VisitTournament visitor);
